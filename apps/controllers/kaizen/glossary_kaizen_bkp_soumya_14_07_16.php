...
class Glossary extends CI_Controller 
{
	...
	public function addedit()
	{
		...
		{	
			...
			{
				$uplod_img ="";			
				$getFile=$this->modelglossary->editDetail($id);
				$orgimgpath=$getFile->picture;
			
				if(is_uploaded_file($_FILES['htmlfile']['tmp_name'])) // if image file upload at the updating
				{
						
					
					if(!empty($orgimgpath) && is_file(file_upload_absolute_path().'glossary/'.$orgimgpath)){
						unlink(file_upload_absolute_path().'glossary/'.$orgimgpath);
					}
				...
				if($this->modelglossary->updateDetais($id,$uplod_img)) // IF UPDATE PROCEDURE EXECUTE SUCCESSFULLY
				{
			...
			else 
			{
				...
				$id = $this->modelglossary->addDetails($uplod_img);
	...
	public function doedit()
	{
		$data = array();
		$glossary_id=$this->uri->segment(4);
		$q = $this->modelglossary->editDetail($glossary_id);		
	...
	public function dodeleteimg() // CHANGE STATUS
	{
		...
		{ 
			if($this->modelglossary->deleteImg($id))
			{				
		...
		$data['details'] = $this->modelglossary->editDetail($id);
