...
class Cms extends CI_Controller {
	...
	public function dolist($searchstring=""){
		$data = array();
		$total_row = $this->modelcms->getCountAll("cms_pages",$searchstring);
		$this->pagination($searchstring,$total_row);
		$data['q'] = $searchstring;
		$data_row = $this->modelcms->getAllDetails("cms_pages",$this->limit,$this->offset,$searchstring);
		if(empty($data_row)){
	...
	public function addedit(){
		...
		{	
			if($this->modelcms->getSingleRecord($id)) 
			{			
				$getFile=$this->modelcms->editDetail($id);
			
			
		
						
				if($this->modelcms->updateDetais($id))
				{
	...
	public function doedit(){
		$data = array();
		$cms_id=$this->uri->segment(4);
		$data['details'] = $this->modelcms->editDetail($cms_id);		
		$data['cms_list']=$this->modelcms->getAllParentDetails();
		$data['hooks_cmsmenu']=$this->get_cms_menu();
		$this->load->view('kaizen/edit_cms',$data);				
	}
	...
	public function doeditajax(){
		...
		$q	= $this->modelcms->editDetail($cms_id);
		if($q){
			$data['details'] = $q;
		}
		else{
