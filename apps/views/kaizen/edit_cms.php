...
function confirmdel(id){
	if(confirm("Are you sure want to delete image?")){
		window.location.href="<?php echo site_url("kaizen/cms/dodeleteimg/");?>?deleteid="+id;
	}
	else{
		return false;
	}
...
$(document).ready(function(){
	...
	?>
	openpage('<?php echo site_url("kaizen/cms/doeditajax/0/");?>');
	<?php
	}
	?>
...
var ContentFromEditor = CKEDITOR.instances.contents.getData();
	...
	if(val=="del"){
		if(confirm("Are you sure want to delete?")){
			window.location.href="<?php echo site_url("kaizen/cms/dodelete/");?>?deleteid=<?php echo $details->id;?>&ref=<?php echo site_url("kaizen/cms/");?>";
		}
		else{
			return false;
		}
...
function goto_page(url){
	if(url==""){
		window.location.href = "<?php echo site_url("kaizen/cms/listing/0/");?>";
	}
	else{
	...
	function setPublish(id)
	{
		...
		if(publish != "")
		{
			$.ajax({
			   type: "POST",
				url : '<?php echo site_url("kaizen/cms/set_publish/");?>',
				data: 'id='+id,
				dataType : "html",
