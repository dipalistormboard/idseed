...
if ( ! function_exists('inputEscapeString'))
{
	function inputEscapeString($str){
		return htmlentities($str, ENT_QUOTES, "UTF-8");
	}
}
if ( ! function_exists('outputEscapeString'))
{	
	function outputEscapeString($str){
...
if(! function_exists('generate_opt_menu')){
	function generate_opt_menu($parent,$menus_array, &$OPT_MENU, $level_depth=0,$selected=""){
		$level_depth++;
		$static_menu_array = array("blog");		
		foreach($menus_array as $key => $value){
			
	    	if ($value['parent_id'] == $parent){
				if(!in_array($value['page_link'],$static_menu_array)){
					
						$OPT_MENU .= '<option value="'.$value['id'].'" '.(($selected==$value['id'])? 'selected="selected"' : '').'>' . $value['title'] . '</option>';
...
if(! function_exists('generate_opt_menu2')){
	function generate_opt_menu2($parent,$menus_array, &$OPT_MENU, $level_depth=0,$selected=""){$level_depth++;
		$static_menu_array = array("blog");		
		foreach($menus_array as $key => $value){
	    	...
	    	if ($value['parent_id'] == $parent){
				if(!in_array($value['page_link'],$static_menu_array)){
					if($level_depth==1){
						$OPT_MENU .= "<optgroup label=''>\n";
						$OPT_MENU .= '<option value="'.$value['id'].'" '.(($selected==$value['id'])? 'selected="selected"' : '').'>' . $value['title'] . '</option>';
						generate_opt_menu2($key,$menus_array,$OPT_MENU,$level_depth,$selected);
						$OPT_MENU .= "</optgroup>\n";
					}
					else{
						$OPT_MENU .= '<option value="'.$value['id'].'" '.(($selected==$value['id'])? 'selected="selected"' : '').'>-->'.$value['title'] . '</option>';
